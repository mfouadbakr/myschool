<div id="app-static" class="pb-2">
      <h4>Food international school</h4>
      <div class="form-inline"><h6 class="mr-1">Student full name</h6><h6 class="text-danger">*</h6></div>
      <div class="row">
        <div class="col-md-3"><span></span></div>
        <div class="col-md-3"><span></span></div>
        <div class="col-md-3"><span></span></div>
        <div class="col-md-3"><span></span></div>
      </div>

      <div class="form-inline"><h6 class="mr-1 mt-2">Gender</h6><h6 class="text-danger">*</h6></div>
      <span>Male</span>

      <hr>

      <div class="form-inline"><h6 class="mr-1">Date of birth</h6><h6 class="text-danger">*</h6></div>
      <span></span>

      <div class="row">
        <div class="col-md-6">
            <div class="form-inline"><h6 class="mr-1 mt-2">Nationality</h6><h6 class="text-danger">*</h6></div>
            <span>Egypt</span>
        </div>
        <div class="col-md-6">
            <div class="form-inline"><h6 class="mr-1">Place of birth</h6><h6 class="text-danger">*</h6></div>
            <span></span>
        </div>
      </div>

      <div class="form-inline"><h6 class="mr-1 mt-2">Religion</h6><h6 class="text-danger">*</h6></div>
            <span>Muslim</span>

              <hr>
        
              <div class="form-inline"><h6 class="mr-1">Address</h6><h6 class="text-danger">*</h6></div>
              <div class="row">
                  <div class="col-md-6">
                      
                      <span></span>
                  </div>
                  <div class="col-md-6">
                      
                      <span></span>
                  </div>
                </div>

                <div class="form-inline"><h6 class="mr-1 mt-2">Home phone</h6><h6 class="text-danger">*</h6></div>
            <span></span>

            <hr>

            <div class="row">
              <div class="col-lg-4">
                  <div class="form-inline"><h6 class="mr-1">Name of school attended last year</h6></div>
                  <span></span>
              </div>
              <div class="col-lg-4">
                  <div class="form-inline"><h6 class="mr-1">Name of 6th prim school</h6></div>
                  <span></span>
                </div>
                <div class="col-lg-4">
                    <div class="form-inline"><h6 class="mr-1">3rd prep school</h6></div>
                    <span></span>
                  </div>
            </div>

            <hr>

            <div class="row">
              <div class="col-md-6">
                <div class="form-inline"><h6 class="mr-1">Father's Job</h6><h6 class="text-danger">*</h6></div>
                  <span></span>
              </div>
              <div class="col-md-6">
                <div class="form-inline"><h6 class="mr-1">Mother's Job</h6><h6 class="text-danger">*</h6></div>
                  <span></span>
              </div>
            </div>

            <hr>

            <div class="mb-5 pb-2 d-none" id="step1-bottom">Must fill all the fields with <h6 class="text-danger" style="display:inline;">*</h6> on them<button class="btn btn-color-2 mb-5 rounded-0 float-right" id="next1">Next</button></div>

            

            
      
    </div><div id="app-dynamic" class="">
        <!--Where controls generated appear-->
        
        
        <!--Where controls generated appear-->
      <div><h6 class="my-2">Fhjhv</h6><div><span></span></div></div><div><h6 class="my-2">Fhjhv</h6><div><span></span></div></div><div><h6 class="my-2">Fhjhv</h6><div><span></span></div></div><div><h6 class="my-2">Fhjhv</h6><div><span></span></div></div><div><h6 class="my-2">Fhjhv</h6><div><span></span></div></div><div><h6 class="my-2">Fhjhv</h6><div><span></span></div></div><div><h6 class="my-2">Fhjhv</h6><div><span></span></div></div>        <hr id="dynamic-hr" class="d-none">

            <div class="mb-5 pb-2 d-none" id="step2-bottom">Must fill all the fields with <h6 class="text-danger" style="display:inline;">*</h6> on them<button class="btn btn-color-2 mb-5 rounded-0 float-right" id="next2">Next</button></div>
      </div>
        <!--Where output of all the application appear-->
        <hr>

            <div class="mb-5 pb-5">Make sure that information is correct<button class="btn btn-color-2 mb-5 rounded-0 float-right" id="finish">Finish</button></div>
      